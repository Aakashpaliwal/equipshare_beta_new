...
module.exports =  {
    ...
    request_this: function(req,res){
        connection.query("SELECT name, mobile, email, address1,address2,address3, city, state, zipcode  FROM account WHERE id = ?",[req.params.owner_id],function(err1,rows1){
            if (err1) throw err1 ;
            else{
                if(user_access){
                ...
                connection.query("SELECT * FROM all_equipment WHERE id = ?",[req.params.id],function(err3,rows3){
                        if (err3) throw err3;
                        else{ 
                            connection.query("SELECT * FROM equipment_type WHERE type_id = ?" ,[rows3[0].type_id], function(err4, rows4){
                                if(err4) throw err4;
                                else res.render('./user_request.ejs', {owner_details : rows1[0] , equip_data : rows3, featured:index_featured, tech_info : rows4[0] , isLoggedIn : isLoggedIn(req,res)});    
                            });
                        }
                });
            }    
        ...
        connection.query("SELECT country FROM account WHERE id = ?", [req.session.user], function(err,rows){
            ...
            if(rows[0].country) return next();
                ...
                connection.query("SELECT * FROM account WHERE id = ?" ,[req.session.user],function(err,rows){
                    if (err) throw err ;
                    else res.render('./user_update_profile.ejs' , {msg : "Please Update your Profile first." , user_data : rows[0]});
                });
            }
        });
    },
    ...
    get_update_profile:  function(req, res){
        connection.query("SELECT * FROM account WHERE id = ?" ,[req.session.user],function(err,rows){
            if (err) throw err ;
            else res.render('./user_update_profile.ejs' , {msg : '' , user_data : rows[0]});
        });
    },
        ...
        connection.query(updateQuery, [req.body.email, req.body.address1,req.body.address2,req.body.address3, req.body.city, req.body.state, 'India', req.body.zipcode, req.session.user],function (err, rows) {
            if (err) throw err;
                ...
                connection.query("SELECT * FROM account WHERE id = ?" ,[req.session.user],function(err1,rows1){
                    if (err) throw err ;
                    else res.render('./user_update_profile.ejs' , {msg : 'Profile Updated' , user_data : rows1[0], isLoggedIn : isLoggedIn(req,res)});
                });          
            }
        });
    },
    ...
    connection.query("SELECT * FROM all_equipment WHERE id = ?" ,[req.params.id],function(err,rows){
            if (err) throw err;
            else res.render('./update.ejs' , {equip_data : rows[0], isLoggedIn : isLoggedIn(req,res)});
        });
    },
         ...
         var updateQuery = "UPDATE all_equipment SET brand =?, model=?, expected_price=?, year=?, colour=?, subcategory=?, description=?, km=? WHERE id =?";
                connection.query(updateQuery, [req.body.brand, req.body.model, req.body.varient, req.body.expected_price, req.body.year, req.body.colour, req.body.subcategory, req.body.description,req.body.km, req.params.id],function (err, rows) {
                    if (err) throw err;
                        ...
                        return res.render('./user_index.ejs',{msg : msg, isLoggedIn : isLoggedIn(req,res)});
                        }
                });
    ...
    my_equipment: function(req , res){
        connection.query("SELECT * FROM all_equipment WHERE owner_id = ?" ,[req.session.user],function(err,rows){
            if (err) throw err ; 
            else res.render('./user_view_equipment.ejs' , {datarows: rows, isLoggedIn : isLoggedIn(req,res)});
        });
    },

    view_equipment:  function(req , res){
        connection.query("SELECT photo, subcategory, brand, model,colour, expected_price, id  FROM all_equipment WHERE available = 1",function(err,rows){
            if (err) throw err ;
            else res.render('./user_view_equipment.ejs' , {datarows: rows, isLoggedIn : isLoggedIn(req,res)});
        });
    },
    ...
    get_add_equipment: function(req,res){
        ...
        connection.query("SELECT DISTINCT subcategory FROM equipment_type WHERE category = ?",[cat_rows[0].category], function(err1, subcat_rows){
            if (err1) throw err1;
            else res.render('./user_add_equipment.ejs', {msg : msg, cat_rows:cat_rows, isLoggedIn : isLoggedIn(req,res)});                             
        });
    },
